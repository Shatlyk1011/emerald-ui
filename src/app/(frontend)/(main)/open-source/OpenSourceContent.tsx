...
const FILTER_CONFIG: { key: FilterKey; title: string; options: { label: string; value: string }[] }[] = [
  { key: 'type', title: 'Project Type', options: OPEN_SOURCE_FILTER_OPTIONS.type },
  { key: 'ui', title: 'UI Framework', options: OPEN_SOURCE_FILTER_OPTIONS.ui },
  { key: 'css', title: 'CSS', options: OPEN_SOURCE_FILTER_OPTIONS.css },
  { key: 'cms', title: 'CMS / Database', options: OPEN_SOURCE_FILTER_OPTIONS.cms },
]
...
const MOCK_COMPONENTS: OpenSourceProject[] = [
  {
    ...
    description: 'A comprehensive Next.js boilerplate with Payload CMS, Stripe integration, and authentication pre-configured.',
    author: 'Antigravity',
  ...
  },
  {
    ...
    description: 'Minimal and clean portfolio template for developers. Includes blog, project showcase, and contact form.',
    author: 'John Doe',
  ...
  },
  {
    ...
    description: 'A full-featured online store template with Supabase, Tailwind CSS, and Stripe checkout.',
    author: 'Shatlyk',
  ...
  },
  {
    ...
    description: 'Feature-rich admin dashboard with Material UI, Prisma, and advanced data visualization charts.',
    author: 'CreativeDev',
  ...
  },
  {
    id: 'mock-5',
    title: 'AI Chatbot UI',
    description: 'A beautiful chat interface for AI applications. Supports multiple models, streaming responses, and markdown rendering.',
    author: 'AILabs',
  ...
  return (
    <div className='flex flex-col gap-12'>
      <div className='flex flex-col gap-4'>
        <h1 className='text-4xl font-bold tracking-tight'>Open Source Projects</h1>
        <p className='text-muted-foreground text-lg'>
          A curated collection of open source projects and boilerplates to jumpstart your next application.
        </p>
      </div>
          ...
          {allComponents && allComponents.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {allComponents.map((component) => (
                <div key={component.id} className="group border border-border rounded-xl flex flex-col h-full bg-card hover:border-primary/50 transition-all overflow-hidden shadow-sm hover:shadow-md">
                  <div className="relative aspect-video w-full overflow-hidden bg-muted/50">
                    {component.imgUrl ? (
                      ...
                      <div className="flex items-center justify-center h-full">
                        <div className="text-muted-foreground/20 font-bold text-4xl select-none">
                  ...
                  <div className="p-6 flex flex-col grow">
                    <div className="mb-4">
                      <h3 className="text-xl font-semibold line-clamp-1 mb-2" title={component.title}>
                        {component.title}
                      </h3>
            ...
            <div className="py-20 text-center">
              ...
              <Button onClick={handleResetFilters} variant="secondary">Reset Filters</Button>
            </div>
          )}
        </section>
      )}
      ...
      <ul className='-ml-1 flex h-max flex-1 flex-wrap place-content-start gap-1 text-base font-normal'>
        {options.map(({ value, label }) => {
          ...
          return (
            ...
            >
              <button
                className='-tracking-one transition-colors ease-in-out text-left'
...
function CustomBadge({ children }: { children: React.ReactNode }) {
  ...
  return (
    <Badge variant="outline" className="capitalize text-sm bg-muted/30 font-normal">
      {children}
    </Badge>
  )
}
