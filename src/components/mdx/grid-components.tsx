...
const GridItem = ({ item, isBlock }: { item: Item; isBlock?: boolean }) => {
  ...
  return (
    <CardWrapper onClick={handleCopyClick} isAuthRequired={isAuthRequired} isCopied={isCopied} isPending={isPending}>
      {item.component}
    </CardWrapper>
  );
};
...
const GridComponents: FC<Props> = ({ items, isBlock }) => {
  return (
    <div className="w-full px-4 pb-40">
      <div className="mx-auto grid w-full max-sm:grid-cols-1 gap-10 grid-cols-2">
        {items.map((btn) => (
...
const CardWrapper = ({
  ...
  return (
    <div className={cn(
        " group/btn relative flex max-md:min-h-40 w-full items-center justify-center overflow-hidden rounded-xl border min-h-60 ",
        className,
      )}>
    {isCopied ? (
        <SuccessParticles
        ...
        <form
          className='w-max absolute top-4 right-4'
          onSubmit={(e) => {
            e.preventDefault()
            onClick?.()
          }}
